...
PRで変更されたファイルを静的に読み、レビュー画面では見落としやすい不可視Unicodeや制御文字を検出します。

- Unicode双方向制御文字（Trojan Source系）をブロック
- ソースコード・設定ファイル中のゼロ幅文字やformat controlをブロック
- Markdownなどの文書中のゼロ幅文字は警告
- ファイル名に含まれる不可視文字もブロック
- 想定外の制御文字や既知テキストファイルの非UTF-8をブロック
- 変更されたsymlinkは内容を辿らずブロック
- バイナリは本文検査の対象外

レポートには問題のある行そのものを出さず、カテゴリ・エスケープ済みファイルパス・行番号だけを表示します。GitHub Actionsではpull requestごとに変更ファイルだけを自動検査します。
